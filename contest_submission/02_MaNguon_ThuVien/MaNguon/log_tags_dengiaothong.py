#!/usr/bin/env python3
"""
log_tags_dengiaothong.py  —  Poll tag PLC hệ thống Đèn Giao Thông (Traffic Light)
==========================================================================
Bảng tag PLC (Traffic Light S7-1200) — Tham chiếu từ bảng tag TIA Portal:
...
Cách dùng:
  python log_tags_dengiaothong.py --target 192.168.1.10 --output logs/day1_dgt_s1_tags.csv
  python log_tags_dengiaothong.py --target 192.168.1.10 --interval 0.5 --output tags.csv
"""
...
# =============================================================================
#  Mapping chính xác từ bảng tag TIA Portal — Đèn Giao Thông
# =============================================================================
...
# Các tổ hợp Q0 hợp lệ đã biết (Running + tối đa 1 pha mỗi hướng, hoặc pha chuyển tiếp Red1+Red2)
VALID_Q0_STATES = {
    0b00000000,  # tất cả tắt (PLC dừng)
    0b00000010,  # Running=1, không đèn nào (khởi động)
    0b00000110,  # Running + Red1
    0b00001010,  # Running + Red2
    0b00001110,  # Running + Red1 + Red2 (chuyển pha)
    0b00100010,  # Running + Yellow1
    0b01000010,  # Running + Yellow2
    0b10000010,  # Running + Green1
    0b01000010,  # Running + Green2 (bit 7, xem VALID_Q0_STATES bên dưới)
}
# Green2 = bit 7 (Q0.7) → thêm tổ hợp Running + Green2
VALID_Q0_STATES.add(0b10000000 | 0b00000010)  # Running + Green2
...
def poll_once(client: snap7.client.Client, poll_seq: int, metadata: dict) -> dict:
    ...
    try:
        ...
        # Phát hiện RWRITE Q0 — q0_raw không khớp với 1 trong các tổ hợp hợp lệ đã biết
        q0_val = row.get('q0_raw', 0)
        row['q_output_unexpected'] = int(q0_val not in VALID_Q0_STATES)
...
def main():
    parser = argparse.ArgumentParser(
        description='PLC Tag Logger — Đèn Giao Thông (Traffic Light) S7-1200',
    ...
    print(f"[LOG_TAGS_DGT] PLC    : {args.target}  rack={args.rack} slot={args.slot}")
    print(f"[LOG_TAGS_DGT] Output : {args.output}")
    print(f"[LOG_TAGS_DGT] Meta   : session={args.session_id} host={args.host_id} day={args.day}")
    print(f"[LOG_TAGS_DGT] Poll   : {args.interval}s ({1/args.interval:.0f} Hz)")
    print(f"[LOG_TAGS_DGT] Hệ thống: Đèn Giao Thông (Traffic Light)")
    print(f"[LOG_TAGS_DGT] Tags   : Q0.1-Q0.7 | M2.1-2.2 | MD3-MD78 | M28.0-3")
    print("[LOG_TAGS_DGT] Ctrl+C để dừng.\n")
    ...
    try:
        ...
        print(f"\n[LOG_TAGS_DGT] Dừng. Tổng poll: {poll_seq}")
        fh.close()
        sys.exit(0)
