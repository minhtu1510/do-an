#!/usr/bin/env bash
# =============================================================================
# run_all_6_days_dengiaothong.sh — Chạy tự động 6 ngày liên tiếp (1 lệnh duy nhất)
# Hệ thống: Đèn Giao Thông (Traffic Light)
# =============================================================================
#
# Cách dùng:
#   bash run_all_6_days_dengiaothong.sh --iface 3
#   bash run_all_6_days_dengiaothong.sh --iface 3 --target 192.168.1.10 --session tl_s2
#   bash run_all_6_days_dengiaothong.sh --iface 3 --days 2 3 4     # chỉ chạy ngày chọn
#   bash run_all_6_days_dengiaothong.sh --iface 3 --profile extended_300k
#
# Log toàn bộ ra file để xem lại:
#   bash run_all_6_days_dengiaothong.sh --iface 3 2>&1 | tee collection_run_tl.log
# =============================================================================
set -uo pipefail
...
usage() {
    cat <<'EOF'
Usage:
  bash run_all_6_days_dengiaothong.sh --iface <NIC> [options]
...
Options:
  ...
  --session ID       Session ID cho dataset (default: tl_s1)
  ...
  --enable-cpu-control  Opt in to CPU_STOP attack (Day 3/6). Default disabled.
...
Examples:
  ...
  bash run_all_6_days_dengiaothong.sh --iface 3 --session tl_s1

  # Chỉ chạy ngày 2 và 3 (đã có ngày 1):
  bash run_all_6_days_dengiaothong.sh --iface 3 --days "2 3"
...
while [[ $# -gt 0 ]]; do
    case "$1" in
        ...
        --enable-cpu-control) EXTRA_ARGS="$EXTRA_ARGS --enable-cpu-control"; shift ;;
        --allow-pa-write-attacks) EXTRA_ARGS="$EXTRA_ARGS --allow-pa-write-attacks"; shift ;;
...
echo ""
echo "╔══════════════════════════════════════════════════════════════╗"
echo "║   ICS DATASET COLLECTION — run_all_6_days_dengiaothong.sh    ║"
echo "╠══════════════════════════════════════════════════════════════╣"
printf "║  Start   : %-49s ║\n" "$START_TIME"
...
COMBINED_SCRIPT="$SCRIPT_DIR/run_combined_day_dengiaothong.sh"

if [[ ! -f "$COMBINED_SCRIPT" ]]; then
    echo "[ERROR] run_combined_day_dengiaothong.sh not found at $COMBINED_SCRIPT" >&2
    exit 1
fi
...
for DAY in $DAYS; do
    ...
    [[ "$EXTRA_ARGS" == *enable-cpu-control* ]] && CMD+=(--enable-cpu-control)
    [[ "$EXTRA_ARGS" == *allow-pa-write-attacks* ]] && CMD+=(--allow-pa-write-attacks)
