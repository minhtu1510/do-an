...
export default function AdminUsers() {
  ...
  return (
    <div className="p-6 space-y-6">
      <PageHeader title="Users" subtitle="Quản lý tài khoản và phân quyền (admin only)." />

      {error && (
        <div className="rounded border border-red-900/50 bg-red-950/30 px-3 py-2 text-sm text-red-400">{error}</div>
      )}

      <form onSubmit={handleCreate} className="flex flex-wrap items-end gap-3 rounded border border-gray-700 bg-gray-800 p-4">
        <label className="flex flex-col gap-1">
          <span className="text-xs uppercase text-gray-500">Username</span>
          <input
            value={form.username}
            onChange={(e) => setForm({ ...form, username: e.target.value })}
            className="rounded border border-gray-700 bg-gray-950 px-3 py-1.5 text-sm text-gray-200 outline-none focus:border-blue-600"
          />
        </label>
        <label className="flex flex-col gap-1">
      ...
      <div className="rounded border border-gray-700 bg-gray-800 overflow-hidden">
        ...
        <div className="divide-y divide-gray-700">
          {users.map((u) => (
            <div key={u.id} className="grid grid-cols-[1fr_140px_180px_100px] items-center gap-3 px-4 py-3">
              <div className="text-sm text-gray-200">{u.username}</div>
              <select
                value={u.role}
                disabled={u.id === session?.id}
                onChange={(e) => handleRoleChange(u.id, e.target.value)}
                className="rounded border border-gray-700 bg-gray-950 px-2 py-1 text-xs text-gray-300 outline-none disabled:opacity-40"
              >
                {ROLES.map((r) => (
                  <option key={r} value={r}>{r}</option>
                ))}
